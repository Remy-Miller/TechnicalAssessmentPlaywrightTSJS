...
test('Booking E2E Test', async ({request}) => {
    ...
    await test.step('Read bookings by query paramaters', async () => {
    ...
    await appendToLogFile(LogFilePath, 'Read bookings by query paramaters Response', readAllBookingsResponse.status(), readAllBookingsResponseBody)
    })
